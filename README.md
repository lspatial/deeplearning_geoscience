...
**问题3.7**  证明式(3.11)满足细致平稳条件，从而抽取的样本最终符合目标概率分布 $p(x)$。<br>**答**： $p(i)q(i,j)\alpha (i,j)=p(i)Q(i,j)\cdot \min \left \{ \frac{p(j)q(j,i)}{p(i)q(i,j)} ,1 \right \}=\min \left \{ p(i)q(i,j),p(j)q(j,i) \right \}=p(j)q(j,i)\cdot \min \left \{ \frac{p(i),q(i,j)}{p(j)q(j,i)},1  \right \}=p(j)q(j,i)\alpha (j,i))$。<br>
