...
**问题3.7**  证明式(3.11)满足细致平稳条件，从而抽取的样本最终符合目标概率分布 $p(x)$。<br>**答**： $p(i)q(i,j)\alpha (i,j)=p(i)Q(i,j)\cdot \min {\left \{ \frac{p(j)q(j,i)}{p(i)q(i,j)} ,1 \right \} }=\min { p(i)q(i,j),p(j)q(j,i) }=p(j)q(j,i)\cdot \min {\{ \frac{p(i),q(i,j)}{p(j)q(j,i)},1\} }=p(j)q(j,i)\alpha (j,i))$。<br>
...
**问题7.2**  根据式（7.15）是否可以得到以下结论，正则化L1所得的解均为正值或0，没有负值？<br>**答**：可以。式（7.15） $\omega _{i}=\max {{\omega _{i}^{\ast }-\frac{\alpha }{H_{ii} },0 } }$表明，当 $\omega _{i}^{\ast }\le \frac{\alpha }{H_{ii} }$ 时， $\omega _{i}=0$；当 $\omega _{i}^{\ast }>\frac{\alpha }{H_{ii} }$时， $\omega _{i}=\omega _{i}^{\ast }-\frac{\alpha }{H_{ii}}$ 。此时的 $\frac{\alpha }{H_{ii}}$相当于一个界限，小于此界线的全部衰减为0，而高于此界线的部分成为新的权重值。所以，L1正则化所得到的解均为正值或0，没有负值。<br>
